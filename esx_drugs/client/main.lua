...
Citizen.CreateThread(function()
	while true do
		...
		if (GetDistanceBetweenCoords(coords, Config.CircleZones.DrugDealer.coords, true) < 0.5) or (GetDistanceBetweenCoords(coords, Config.CircleZones.DrugDealer2.coords, true) < 0.5) or (GetDistanceBetweenCoords(coords, Config.CircleZones.DrugDealer3.coords, true) < 2) then
...
function OpenDrugShop()
	...
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
	local DrugDealerlocation = 0

	if GetDistanceBetweenCoords(coords, Config.CircleZones.DrugDealer.coords, true) < 0.5 then
		DrugDealerlocation = 1
	elseif GetDistanceBetweenCoords(coords, Config.CircleZones.DrugDealer2.coords, true) < 0.5 then
		DrugDealerlocation = 2
	elseif GetDistanceBetweenCoords(coords, Config.CircleZones.DrugDealer3.coords, true) < 2 then
		DrugDealerlocation = 3
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'drug_shop', {
		...
		TriggerServerEvent('esx_drugs:sellDrug', data.current.name, data.current.value, DrugDealerlocation)
